...
#Continuo solo con variables percentiladas
# Seleccionar columnas que contienen 'percentile' o son específicas
columnas_a_conservar = ['numero_de_cliente', 'foto_mes', 'clase_ternaria']
data = data.loc[:, data.columns.str.contains('percentile') | data.columns.isin(columnas_a_conservar)]
...
for mes in meses_train:
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name=f"optuna_lgbm_sinPer_{mes}",
        storage=f"sqlite:///optuna_sinPer_{mes}.db",
        load_if_exists=True
    )
...
plot_filename_local = os.path.join(local_directory, 'ganancia_por_mes_SoloPercentiles.png')
csv_filename_local = os.path.join(local_directory, 'ganancias_SoloPercentiles.csv')
